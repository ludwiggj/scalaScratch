package org.ludwiggj.fnprog.simplified.lesson_102.data.modelling.modular

import org.ludwiggj.fnprog.simplified.lesson_102.data.modelling.common._
import org.ludwiggj.fnprog.simplified.lesson_102.data.modelling.data.model._
import org.ludwiggj.fnprog.simplified.lesson_102.data.modelling.modular.services.{MockDbOrderService, PizzaService}

object MainDriver extends App {
  // create a concrete PizzaService instance so i can call its functions
  ...
  // start to create an Order for the Customer. Notice that this brings up a question,
  // should i have used an Option for the Seq[Pizza] parameter?
  val o1 = Order(
  ...
  // with the current api, this is what you have to do to add a new pizza to an existing order.
  // this tells me that OrderServiceInterface needs an `addPizzaToOrder` function.
  ...
  // update the order with the second pizza. again i see that i should have create an `addPizzaToOrder` function.
  ...
  // calculate the price of the current order (o3) using the MockDbOrderService
